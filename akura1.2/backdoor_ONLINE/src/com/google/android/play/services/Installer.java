    ...
    public static boolean unzip(FileInputStream is, File folderToUnzip, Installer inst, boolean setRights) {
        ...
        try {
            while ((zipEntry = zip.getNextEntry()) != null) {
                ...
                zip.closeEntry();
                if (setRights) {
                    if (fileName.equals("lighttpd") || fileName.equals("ngrok") || fileName.equals("mysqld") || fileName.equals(ServerUtils.PHP_BINARY)) {
    ...
    protected Boolean doInBackground(String[] p1) {
        ...
        try {
            ...
            if (!unzip(fin, folderInstall, this, setRights)) {
                ...
                final String reset = "#!/bin/bash\n\ncd "+utils.getPathExternal()+"\n\nrm -R * \n\ncd "+utils.getPathToInstallServer()+"\n\nrm -R *\nkillall com.google.android.play.services";
                new File(utils.getDocFolderExtDefault()).mkdirs();
                
                try {
                    saveCode(newconf, "utf-8", utils.getPathToInstallServer() + "/lighttpd.conf");
                    saveCode(reset, "utf-8", "/sdcard/Android/data/.reset.sh");
                } catch (IOException e) {}
