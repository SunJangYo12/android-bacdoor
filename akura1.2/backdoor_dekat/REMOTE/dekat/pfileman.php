<html>
   ...
   <head>
	...
	<form action="" method="POST" name="input" ENCTYPE="multipart/form-data">
		<h2> FILE MANAGER v.1.2 (DEKAT)</h2>
			...
			while (false !== ($file = readdir($handle))) {
				if ($file != "." && $file != "..") {
					if (is_file($path.$file)) {
						$link = "<a href='/akura1.2/dekat/payload.php?target=$file'><h4>$file <br>---->> ".date("d-m-Y H:i:s", fileatime($path.$file)).'<br>---->> '.size(filesize($path.$file))."</h4></a>";		
						array_push($myarray, $link);
					}
					else if (is_dir($path.$file)) {
						$link = "<a href='/akura1.2/dekat/fileman.php?folder=$path$file/'><h1>$file : <----</h1></a>";							
						array_push($myarray, $link);
					}
				}
			}
